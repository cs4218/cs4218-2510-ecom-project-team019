...
describe('forgotPasswordController integration', () => {
    ...
    it('should reset password when security answer matches', async () => {
        ...
        const matchesOld = await comparePassword(
            'OldPassword123',
...
describe('integration test: getOrdersController', () => {
    ...
    it('should return 200 and the list of orders for the user', async () => {
        ...
        const orders = jsonArg.orders;

        expect(orders.length).toBe(1);
        expect(orders[0].buyer.name).toBe('John Doe');
        expect(orders[0].products.length).toBe(2);

        // Check product details are populated
        expect(orders[0].products[0]).toHaveProperty('name', 'Book');
        expect(orders[0].products[1]).toHaveProperty('name', 'Laptop');
    });
    ...
    it('should return 200 and empty array if no orders exist', async () => {
        ...
        expect(res.status).toHaveBeenCalledWith(200);
        const jsonArg = res.json.mock.calls[0][0];
        expect(Array.isArray(jsonArg.orders)).toBe(true);
        expect(jsonArg.orders.length).toBe(0);
    });
